...
public class PanelCliente extends JPanel {
    ...
    private JPanel crearSubRegistro() {
        ...
        btnOk.addActionListener(e -> {
            try {
                ...
                int id = Integer.parseInt(txtId.getText().trim());
                String pw = new String(txtPw.getPassword());
                if (nombre.isEmpty() || pw.isEmpty()) {
                    MensajesCliente.reservaError();
                    return;
                }
    ...
    private JPanel crearSubMenuLogueado() {
        ...
        g.gridy = 0;
        p.add(new JLabel("── Menú Cliente ──", SwingConstants.CENTER), g);
        g.gridy = 1;
        p.add(btnReservar, g);
        g.gridy = 2;
        p.add(btnMisReservas, g);
        g.gridy = 3;
        p.add(btnSalir, g);
        return p;
    }
    ...
    private JPanel crearSubFiltro() {
        ...
        modeloTabla = new DefaultTableModel(new String[]{"ID", "Cancha", "Hora", "Fecha", "Capacidad"}, 0) {
            @Override
    ...
    private JPanel crearSubMisReservas() {
        JPanel p = new JPanel(new BorderLayout(8, 8));
        p.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
        modeloMisReservas = new DefaultTableModel(new String[]{"ID", "Cancha", "Hora", "Fecha", "Capacidad"}, 0) {
            @Override
            public boolean isCellEditable(int r, int c) {
                return false;
            }
    ...
    private void actualizarMisReservas() {
        ...
        List<Reserva> lista = ServicioReserva.getReservasDeCliente(clienteActual.getId());
        for (Reserva r : lista) {
            modeloMisReservas.addRow(new Object[]{r.getIdReserva(), r.getCancha(), r.getHora(), r.getFecha(), r.getCapacidad()});
        }
    }
