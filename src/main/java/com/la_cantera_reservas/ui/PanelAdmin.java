...
public class PanelAdmin extends JPanel {
    ...
    private JPanel crearVistaLogin() {
        ...
        g.gridx=0; g.gridy=0; form.add(new JLabel("ID Admin:"),   g);
        g.gridx=1;             form.add(txtId,                      g);
        g.gridx=0; g.gridy=1; form.add(new JLabel("Contraseña:"), g);
        g.gridx=1;             form.add(txtPw,                      g);
        g.gridx=0; g.gridy=2; g.gridwidth=2; form.add(btnLogin,    g);
        g.gridy=3;             form.add(lblError,                   g);
    ...
    private JPanel crearTabConsultar() {
        ...
        top.add(new JLabel("ID del cliente:")); top.add(txtIdCliente); top.add(btnBuscar);
        ...
        btnBuscar.addActionListener(e -> {
            ...
            try {
                ...
                Reserva r  = ServicioReserva.getReservaDeCliente(id);
                if (r != null) {
    ...
    private JPanel crearTabCrear() {
        ...
        g.gridx=0; g.gridy=0; form.add(new JLabel("Cancha:"),    g);
        g.gridx=1; g.gridwidth=3; form.add(cmbCancha,             g);
        g.gridwidth=1;
        g.gridx=0; g.gridy=1; form.add(new JLabel("Hora:"),      g);
        g.gridx=1; g.gridwidth=3; form.add(cmbHora,               g);
        g.gridwidth=1;
        g.gridx=0; g.gridy=2; form.add(new JLabel("Fecha:"),     g);
        g.gridx=1; form.add(spinAnio, g); g.gridx=2; form.add(spinMes, g); g.gridx=3; form.add(spinDia, g);
        g.gridx=1; g.gridy=3; form.add(new JLabel("año"), g); g.gridx=2; form.add(new JLabel("mes"), g); g.gridx=3; form.add(new JLabel("día"), g);
        g.gridx=0; g.gridy=4; form.add(new JLabel("Capacidad:"), g);
        g.gridx=1; g.gridwidth=3; form.add(txtCap,                g);
        g.gridx=0; g.gridy=5; g.gridwidth=4; form.add(btnCrear,   g);
    ...
    private JPanel crearTabEliminar() {
        ...
        btnEliminarDisp.addActionListener(e -> {
            ...
            if (JOptionPane.showConfirmDialog(this, "¿Eliminar reserva #" + id + "?",
                    "Confirmar", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                if (ServicioReserva.eliminarReservaDisponible(id)) refrescarTablaDisp(modeloDisp);
                else MensajeAdmin.reservaError();
            }
        });
    ...
    private void actualizarCapacidad(JComboBox<Cancha> cmb, JTextField txt) {
        ...
        if (c == null) return;
        int cap;
        if      (c.getTipo().equals("Fútbol 7"))  cap = 14;
        else if (c.getTipo().equals("Fútbol 11")) cap = 22;
        else                                       cap = 10;
    ...
    private void refrescarTablaActivas(DefaultTableModel m) {
        m.setRowCount(0);
        for (Map.Entry<Integer, Reserva> e : ServicioReserva.getReservasActivas().entrySet()) {
